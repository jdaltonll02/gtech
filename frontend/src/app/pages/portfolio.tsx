...
interface Education { id: string; institution: string; degree: string; field_of_study: string; start_year: string; end_year?: string; gpa?: string; description?: string; }
  ...
  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-black/[0.02] rounded-3xl">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h2 className="text-4xl">Skills & Expertise</h2>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {entries.map(([category, items], catIdx) => {
            const Icon = getCategoryIcon(category);
            const pillColor = PILL_COLORS[catIdx % PILL_COLORS.length];
            return (
              <motion.div
                key={category}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {items.map((skill, skillIdx) => (
                      <motion.span
                        ...
                        viewport={{ once: true }}
                        transition={{ duration: 0.28, delay: catIdx * 0.07 + skillIdx * 0.03 }}
                        whileHover={{ scale: 1.08, y: -1 }}
                        className={`px-3 py-1 text-xs font-medium border rounded-full cursor-default transition-shadow hover:shadow-sm ${pillColor}`}
                      >
                        {skill}
                      </motion.span>
                    ))}
                ...
                <div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    ...
                    <a href="/resume" target="_blank" rel="noopener noreferrer">
                      <Button size="lg" variant="outline">
                        <FileText className="mr-2 w-5 h-5" />Export CV
                      </Button>
                    </a>
                  </div>
                </div>
              ...
              <div className="max-w-4xl mx-auto">
                ...
                <Timeline
                  entries={experiences.map((exp) => ({
                    ...
                    extra: exp.achievements?.length > 0 ? (
                      <ul className="mt-3 space-y-1.5">
                        {exp.achievements.map((a, i) => (
                          <li key={i} className="flex items-start text-xs text-black/55">
                            <span className="text-primary mr-2 mt-0.5 flex-shrink-0">▸</span>{a}
                          </li>
                        ))}
                      </ul>
                    ) : undefined,
              ...
              <div className="max-w-4xl mx-auto">
                ...
                <div className="space-y-6">
                  {educations.map((edu, index) => (
                    <motion.div key={edu.id} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: index * 0.1 }}>
                      <GlassCard className="p-6" hover>
                        <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-3">
                          <div>
                            <h3 className="text-xl text-primary">{edu.institution}</h3>
                            <p className="text-lg">{edu.degree} in {edu.field_of_study}</p>
                          </div>
                          <div className="text-sm text-black/50 mt-2 md:mt-0 md:text-right">
                            <p>{edu.start_year} – {edu.end_year ?? 'Present'}</p>
                            {edu.gpa && <p>GPA: {edu.gpa}</p>}
                          </div>
                        </div>
                        {edu.description && <p className="text-black/70 text-sm">{edu.description}</p>}
                      </GlassCard>
                    </motion.div>
                  ))}
                </div>
